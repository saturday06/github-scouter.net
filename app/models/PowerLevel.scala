 ...
 */
object PowerLevels {
  def findOrCreateByUserName(userName: String)(implicit session: Session): Option[PowerLevel] = {
    // TODO: sortByをいちいち書くのは必要か？
    ...
    query.filter(_.timestamp > fromTimestamp).sortBy(_.timestamp.desc).firstOption.map { found =>
      return Some(found)
    }
    ...
    // 全検索して返す
    query.sortBy(_.timestamp.desc).firstOption
  }
  ...
  def executeScouter(userName: String): Option[PowerLevel] = {
    ...
    val process = Process(Seq(
      ...
      "--no-cache",
      "--token",
      Play.application.configuration.getString("githubscouter.githubapi.token"),
      "--api-url",
      Play.application.configuration.getString("githubscouter.githubapi.url")
